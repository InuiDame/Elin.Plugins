...
namespace GBF.spell.Spell_Total_Eclipse
{
    // why I use 2616 to 2612?
    internal class SpellGBF_2616 : Spell
{
    ...
    public override bool Perform()
    {
        ...
        SpellGBF_2616.ProcAt_Internal(element?.Value ?? 1, base.source, base.act);
    ...
    // 内部攻击处理 - 十字形法术 / Internal attack processing - Cross-shaped spell / 内部攻撃処理 - 十字形スペル
    private static void ProcAt_Internal(int power, SourceElement.Row source, Act act)
    {
        ...
        Point origin = Act.TP;
        const int crossRadius = 2; // 横竖各 1 格 → 3×3 的十字 / Horizontal and vertical 1 tile each → 3×3 cross / 縦横各1タイル → 3×3の十字
        ...
        Act.CC.Chara.Say("Spell_GBF_2612", Act.CC.Chara, element.Name.ToLower(), null);
