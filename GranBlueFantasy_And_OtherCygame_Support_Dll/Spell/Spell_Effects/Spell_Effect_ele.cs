...
namespace GBF.spell.Spell_Effects.Spell_Effect_ele
{
   internal static class GBFHandSpellEffects
{
    // 执行手部法术攻击效果 / Perform hand spell attack effects / 手魔法攻撃効果を実行
    internal static void Atk(Chara cc, Point tp, int power, Element element, List<Point> targets, Act act, string alias, float exDelay = 0f, bool rev = false, string spellType = "ball_")
    {
        ...
        foreach (Point point in targets)
        {
            ...
            if (!(spellType == "ball_") && !(spellType == "bolt_"))
            {
                ...
                {
                    effect = Effect.Get("Element/eleDarkness");  // 默认黑暗特效 / Default darkness effect / デフォルト闇エフェクト
                }
                else
                {
            ...
            if (rev)
            {
                num4 = 0.15f / (float)num3;  // 反向延迟计算 / Reverse delay calculation / 逆方向遅延計算
            }
            else
            {
