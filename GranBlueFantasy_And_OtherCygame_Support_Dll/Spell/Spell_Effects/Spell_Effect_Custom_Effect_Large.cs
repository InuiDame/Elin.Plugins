...
namespace GBF.spell.Spell_Effects.Spell_Effect_Custom_Effect_Large
{
    ...
    /// 
    /// 2612和2701本质都是一样的，在命中目的地播放一个大的特效，而不是在每个命中点位播放一个特效
    /// 2612 and 2701 are essentially the same, playing one large effect at the hit destination rather than playing an effect at each hit point
    /// 2612と2701は本質的に同じで、各ヒットポイントでエフェクトを再生するのではなく、ヒット先で1つの大きなエフェクトを再生します
    internal static class GBFSpellEffects_2612
{
    // 执行2612类型法术攻击效果 / Perform 2612 type spell attack effects / 2612タイプ魔法攻撃効果を実行
            ...
            foreach (Card card2 in hits)
            {
                ...
                if (target != null && cc.Chara.IsFriendOrAbove(target))
                {
                    ...
                    if (num7 > 0)
                    {
                        ...
                        Debug.Log($"GBFSpell2612: controlLv:{num7 * 10}, dmg:{EClass.rnd(num6 + 1)}");  // 控制等级和伤害日志 / Control level and damage log / 制御レベルとダメージログ
                ...
                cc.Say("Spell_GBF_2612_hit", cc, card2, element.Name.ToLower(), null);  // 播放命中语音 / Play hit voice / 命中音声を再生
            }
        }
    }
}
    internal static class GBFSpellEffects_2701
{
    // 执行2701类型法术攻击效果 / Perform 2701 type spell attack effects / 2701タイプ魔法攻撃効果を実行
    internal static void Atk(Chara cc, Point tp, int power, Element element, List<Point> targets, Act act, string alias, float exDelay = 0f, bool rev = false, string spellType = "ball_")
    {
            ...
            foreach (Card card2 in hits)
            {
                ...
                if (target != null && cc.Chara.IsFriendOrAbove(target))
                {
                    ...
                    if (num7 > 0)
                    {
                        ...
                        Debug.Log($"GBFSpell2701: controlLv:{num7 * 10}, dmg:{EClass.rnd(num6 + 1)}");  // 控制等级和伤害日志 / Control level and damage log / 制御レベルとダメージログ
                ...
                cc.Say("Spell_GBF_2701_hit", cc, card2, element.Name.ToLower(), null);  // 播放命中语音 / Play hit voice / 命中音声を再生
            }
        }
    }
}
